...
# Función para calcular una curva de Bézier cúbica
def draw_bezier_curve(x0, y0, x1, y1, x2, y2, x3, y3, steps=100):
...
def draw_arm(angle1, angle2):
    ...
    # Dibujar brazo inferior (línea recta)
    x1, y1 = x0, y0 - length1
    draw_line(x0, y0, x1, y1)

    # Dibujar el brazo superior (línea recta o curva)
    rad1 = math.radians(angle1)
    x2 = int(x1 + length2 * math.cos(rad1))
    y2 = int(y1 - length2 * math.sin(rad1))
    
    # Curva Bézier para representar la conexión entre los brazos
    draw_bezier_curve(x1, y1, x1 + 5, y1 - 5, x2 - 5, y2 + 5, x2, y2)

    # Brazo final 
    rad2 = math.radians(angle2)
    x3 = int(x2 + length3 * math.cos(rad1 + rad2))
    y3 = int(y2 - length3 * math.sin(rad1 + rad2))
    draw_bezier_curve(x2, y2, x2 + 5, y2 - 5, x3 - 5, y3 + 5, x3, y3)
